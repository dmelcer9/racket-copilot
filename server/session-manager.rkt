...
(struct sess-client-connect [client-info])
(struct sess-client-disconnect [client-id])
(struct sess-client-msg [client-id msg])


; A SessionState is one of
...
(struct session-state [clients current-host doc-info])
(struct session-state-end [])
...
(struct no-document [])
(struct document [base-rev incremental-edits])
...
(module+ test
  ...
  (check-equal? (server (sess-client-connect c2) newly-created-server-state)
                (values (list (sess-client-msg 1 (msg-participant-info c1 (list c2) 1))
  ...
  (check-equal? (server (sess-client-msg 2 (msg-base-revision b0)) server-state-after-first-join)
                (values (list (sess-client-msg 2 (msg-error (error-not-allowed-by-role))))
                        server-state-after-first-join))
  ...
  (check-equal? (server (sess-client-msg 1 incr-edit-1) server-state-after-first-join)
                (values (list (sess-client-msg 1 (msg-error (error-illegal-state))))
                        server-state-after-first-join))

  ; Send base revision
  (check-equal? (server (sess-client-msg 1 (msg-base-revision b0)) server-state-after-first-join)
                (values (list (sess-client-msg 2 (msg-base-revision b0)))
  ...
  (check-equal? (server (sess-client-msg 1 incr-edit-1) server-state-base-revision)
                (values (list (sess-client-msg 2 incr-edit-1)) server-state-one-revision))
  (check-equal? (server (sess-client-msg 1 incr-edit-2) server-state-one-revision)
                (values (list (sess-client-msg 2 incr-edit-2)) server-state-two-revisions))

  ; Client joins after base and some revisions
  (check-equal? (server (sess-client-connect c3) server-state-two-revisions)
                (values (list (sess-client-msg 1 (msg-participant-info c1 (list c2 c3) 1)) ; Participant infos
  ...
  (check-equal? (server (sess-client-msg 1 (msg-base-revision b1)) server-state-two-revisions-three-clients)
                (values (list (sess-client-msg 2 (msg-base-revision b1))
  ...
  (check-equal? (server (sess-client-msg 1 incr-edit-3) server-state-two-revisions-three-clients)
                (values (list (sess-client-msg 2 incr-edit-3)
  ...
  (check-equal? (server (sess-client-msg 1 (msg-transfer-host 2)) server-state-base-revision-three-clients)
                (values (list (sess-client-msg 1 (msg-participant-info c1 (list c2 c3) 2))
