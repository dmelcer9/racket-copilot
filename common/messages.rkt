...
(struct client-info [name id])
(struct session-info [id pass])

(struct error-version-mismatch [expected-version])
(struct error-not-allowed-by-role [])
(struct error-illegal-state [])

(struct msg-new-session [name version])
(struct msg-join-session [name version session-info])
(struct msg-base-revision [file])
(struct msg-incremental-edit [edit-event])
(struct msg-transfer-host [new-user])
(struct msg-request-new-base-revision [])
(struct msg-error [error-info])
(struct msg-new-session-info [you session-info])
(struct msg-participant-info [you others host-id])